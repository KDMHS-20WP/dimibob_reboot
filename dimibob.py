...
for bob_article in bob_articles:
  ...
  if bob_title.find('식단') != -1:
    ...
    if(found_date.year != 1):
      print(found_date.strftime("%m/%d 식단표"))
      ...
      josik = bob_menu.partition('*조식 : ')[2].partition('*중식 : ')[0].replace('/', '\n')
      jungsik = bob_menu.partition('*중식 : ')[2].partition('*석식 : ')[0].replace('/', '\n')
      soksik = bob_menu.partition('*석식 : ')[2].replace('/', '\n')
